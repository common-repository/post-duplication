...
function pdwp_duplicate_post( $original_id, $args=array(), $do_action=true ) {
	...
	$global_settings = get_pdwp_post_duplication_settings();
  ...
  if( $do_action ) {
  	do_action( 'pdwp_post_duplication_created', $original_id, $duplicate_id, $settings );
  }
...
function p4d_duplicate_post() {

	// variables Get
	$original_id  = $_POST['original_id'];
	
	// The post Duplicate 
	$duplicate_id = pdwp_duplicate_post( $original_id );

	//Nonce Check 
	check_ajax_referer( 'p4d_ajax_file_nonce', 'security' );
	
	echo $duplicate_id;

	die(); // Die function for proper result
}
add_action( 'wp_ajax_p4d_duplicate_post', 'p4d_duplicate_post' );